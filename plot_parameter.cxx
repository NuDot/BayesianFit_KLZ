...
void plot(std::map<std::string, TH1D*> MCMap, TH1D* hist_data, std::vector<double> paramVector, int rbin, int thetabin) {
    ...
    histcontainer[0]->SetAxisRange(0.1, 100000.0,"Y");
    ...
    if (foundzerovbb) {
        ...
        for (int i = 1; i <= nbins; ++i) {
            cout<< zerovbbhist->GetBinContent(i) <<"}";
    ...
    gr3->GetYaxis()->SetRangeUser(-1.0,3.0);
    gr3->GetXaxis()->SetLimits(0.5,4.8);
...
int main()
{
    ...
    // m.WriteMarkovChain("chain.root", "UPDATE");
    ...
    std::vector<double> bestFit = dest;

    cout<<m.GetDataCountsROI()<<endl;
    cout<<"+============="<<endl;

    // std::vector<double> bestFit = m.GetBestFitParameters();
    std::vector<string> fitParamName = m.GetFittedRateParameter();
    double total = 0.0;
    for (int i=0;i<fitParamName.size();i++) {
        double current_bkg = m.GetBackgroundCountsROI(fitParamName[i],bestFit);
        cout<<fitParamName[i]<<": "<<bestFit[m.GetFitParameterIndex(fitParamName[i])]<<"  "<<current_bkg<<endl;
        total += current_bkg;
    }
    cout<<total<<endl;
    cout<<"+============="<<endl;
    ...
    // // m.GetParameter("Rate_Xe136_0nu_XeLS").Fix((atof(getenv("ZERONU_RATE"))));
    // m.GetParameter("Rate_Xe136_0nu_XeLS").Unfix();

    // m.WriteMarkovChain("chain_update.root", "RECREATE");

    // // std::vector<double> scale;
    // // m.SetFlagPreRun(true);
    // // scale.push_back(5E-1);
    // // m.SetInitialScaleFactors(scale);
    // // m.SetNIterationsRun(500);
    // // m.SetNIterationsPreRunCheck(100);
    // // m.SetScaleFactorLowerLimit(1E-12);
    // // // m.SetInitialPositions(m.GetInitialPosition());
    // // m.SetProposeMultivariate(true);
    // // // m.SetProposalFunctionDof(24); // Student's t with 5 degrees of freedom
    // // m.SetProposalFunctionDof(-1);

    // // m.SetNIterationsPreRunMin(1000);
    // // m.SetNIterationsPreRunMax(2000);
    ...
    // // m.SetMinimumEfficiency(0.19);
    // // m.SetMaximumEfficiency(0.41);
    // m.SetNIterationsPreRunCheck(15);
    // m.SetScaleFactorLowerLimit(1E-9);
